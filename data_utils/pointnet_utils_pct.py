...
def sample_and_group(npoint, radius, nsample, xyz, points, returnfps=False, knn=False):
    ...
    torch.cuda.empty_cache()
    grouped_xyz_norm = grouped_xyz - new_xyz.view(B, S, 1, C) # 32,256,16,3
    torch.cuda.empty_cache()

    if points is not None:
        grouped_points = index_points(points, idx) # 32,256,16,32
        new_points = torch.cat([grouped_xyz_norm, grouped_points], dim=-1) # [B, npoint, nsample, C+D] 32,256,16,35
    else:
...
class PointNetSetAbstraction_pct(nn.Module):
    def __init__(self, npoint, radius, nsample, in_channel, mlp, group_all, knn=False):
        super(PointNetSetAbstraction_pct, self).__init__()
        self.npoint = npoint
        self.radius = radius
