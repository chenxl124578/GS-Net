...
# from train_classifier import ROOT_DIR
# sys.path.append(ROOT_DIR)
# sys.path.append(os.path.join(ROOT_DIR,"Pointnet2_PyTorch/pointnet2_ops_lib"))
...
class SoftProjection(nn.Module):
    def __init__(
        ...
        self._temperature = torch.nn.Parameter(
            torch.tensor(
                initial_temperature,        # 1.0
                requires_grad=is_temperature_trainable, # true
                dtype=torch.float32,    
            )
        )

        self._min_sigma = torch.tensor(min_sigma, dtype=torch.float32)  # 0.01
    ...
    def _group_points(self, point_cloud, query_cloud, point_features=None):
        ...
        idx = idx.contiguous()    # debug,idx must be contiguous
    ...
    def project(self, point_cloud, query_cloud, hard=False):
        grouped_points, _ = self._group_points(point_cloud, query_cloud)  # [B,3,querypoint,K]
        dist = self._get_distances(grouped_points, query_cloud)  # 邻居点到中心点距离

        # pass through softmax to get weights
        weights = torch.softmax(-dist, dim=_axis_to_dim(2))  # 取负后近的点softmax后值大，意思是距离近的点权重更大
        if hard:
